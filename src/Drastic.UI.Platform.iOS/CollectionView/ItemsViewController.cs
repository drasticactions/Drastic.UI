	...
	where TItemsView : ItemsView
	{
		...
		protected void RemeasureLayout(VisualElement formsElement)
		{
			if (IsHorizontal)
			{
				...
				Drastic.UI.Layout.LayoutChildIntoBoundingRegion(formsElement, new Rectangle(0, 0, request.Request.Width, CollectionView.Frame.Height));
			}
			else
			{
				var request = formsElement.Measure(CollectionView.Frame.Width, double.PositiveInfinity, MeasureFlags.IncludeMargins);
				Drastic.UI.Layout.LayoutChildIntoBoundingRegion(formsElement, new Rectangle(0, 0, CollectionView.Frame.Width, request.Request.Height));
			}
		}
