...
namespace Drastic.UI.Platform.iOS
{
		...
		where TViewController : ItemsViewController<TItemsView>
	{
		...
		protected override void OnElementPropertyChanged(object sender, PropertyChangedEventArgs changedProperty)
		{
			...
			if (changedProperty.Is(Drastic.UI.ItemsView.ItemsSourceProperty))
			{
				UpdateItemsSource();
			}
			else if (changedProperty.Is(Drastic.UI.ItemsView.ItemTemplateProperty))
			{
				UpdateLayout();
			}
			else if (changedProperty.IsOneOf(Drastic.UI.ItemsView.EmptyViewProperty,
				Drastic.UI.ItemsView.EmptyViewTemplateProperty))
			{
				Controller.UpdateEmptyView();
			}
			else if (changedProperty.Is(Drastic.UI.ItemsView.HorizontalScrollBarVisibilityProperty))
			{
				UpdateHorizontalScrollBarVisibility();
			}
			else if (changedProperty.Is(Drastic.UI.ItemsView.VerticalScrollBarVisibilityProperty))
			{
				UpdateVerticalScrollBarVisibility();
			}
			else if (changedProperty.Is(Drastic.UI.ItemsView.ItemsUpdatingScrollModeProperty))
			{
