	...
	public class TabletFlyoutPageRenderer : UISplitViewController, IVisualElementRenderer, IEffectControlProvider
	{
		...
		public override void ViewDidLayoutSubviews()
		{
			...
			if (_previousViewDidLayoutSize != View.Bounds.Size)
			{
				...
				if (FlyoutPage.CanChangeIsPresented && FlyoutPage.IsPresented != IsFlyoutVisible)
					ElementController.SetValueFromRenderer(Drastic.UI.FlyoutPage.IsPresentedProperty, IsFlyoutVisible);
			}
		...
		void HandlePropertyChanged(object sender, PropertyChangedEventArgs e)
		{
			...
			if (e.PropertyName == "Flyout" || e.PropertyName == "Detail")
				UpdateControllers();
			else if (e.PropertyName == Drastic.UI.FlyoutPage.IsPresentedProperty.PropertyName)
				ToggleFlyout();
			else if (e.PropertyName == Drastic.UI.FlyoutPage.IsGestureEnabledProperty.PropertyName)
				...
				UpdateFlowDirection();
			else if (e.Is(Drastic.UI.FlyoutPage.FlyoutLayoutBehaviorProperty))
		...
		void FlyoutControllerDidAppear(object sender, EventArgs e)
		{
			if (FlyoutPage.CanChangeIsPresented && IsFlyoutVisible)
				ElementController.SetValueFromRenderer(Drastic.UI.FlyoutPage.IsPresentedProperty, true);
		}

		void FlyoutControllerWillDisappear(object sender, EventArgs e)
		{
			if (FlyoutPage.CanChangeIsPresented && !IsFlyoutVisible)
				ElementController.SetValueFromRenderer(Drastic.UI.FlyoutPage.IsPresentedProperty, false);
		}
