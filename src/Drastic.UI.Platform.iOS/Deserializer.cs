			...
			return Task.Run(() =>
			{
				...
				using (var reader = XmlDictionaryReader.CreateBinaryReader(stream, XmlDictionaryReaderQuotas.Max))
				{
					...
					catch (Exception e)
					{
						...
						Log.Warning("Drastic.UI PropertyStore", $"Exception while reading Application properties: {e}");
					}
				}
				...
				using (var store = IsolatedStorageFile.GetUserStoreForApplication())
				{
					...
					using (var writer = XmlDictionaryWriter.CreateBinaryWriter(stream))
					{
						...
						catch (Exception e)
						{
							...
							Log.Warning("Drastic.UI PropertyStore", $"Exception while writing Application properties: {e}");
							return;
						}
					}
